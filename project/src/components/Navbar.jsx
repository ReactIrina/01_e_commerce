...
const Navbar = () => {
  ...
  return (
    <header className="max-w-screen-2xl xl:px-28 px-4 mx-auto absolute top-0 left-0 right-0">
      <nav className="flex justify-between items-center container md:py-4 pt-6 pb-3">
        ...
        <div className="sm:hidden">
          <button onClick={toggleMenu}>
            {isMenuOpen ? (
              <FaTimes className="w-6 h-6 text-Icons" />
            ) : (
      ...
      <div className="pt-4">
        <ul className="lg:flex items-center justify-between text-Icons hidden">
          {navItems.map(({ title, path }) => (
            <li key={title} className=" hover:text-Brand">
              <NavLink
                to={path}
      ...
      <div>
        <ul
          className={`bg-Icons text-white px-4 py-2 rounded ${
            isMenuOpen ? "" : "hidden"
          }`}
        >
          {navItems.map(({ title, path }) => (
            <li key={title} className=" hover:text-Brand my-3 cursor-pointer">
